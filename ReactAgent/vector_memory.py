...
class React_Agent:
    ...
    def print_chat_history(self, chat_memory_buffer) -> None:
        """Print chat history from memory"""
        print("\nChat History:")
        for idx, entry in enumerate(chat_memory_buffer.get_history()):
            print(f"{idx + 1}. {entry['type'].capitalize()}: {entry['content']}")
...
def main():
    ...
    # Print chat history
    agent_instance.print_chat_history(agent_instance.chat_memory_buffer)
